...
protected:
	...
	int Threshold(Mat image, int gen);
...
public:
	...
	Mat ImageplusSeg(Mat image1, Mat image2);
	void ImageplusSegMat(Mat image1, Mat image2);
	void GateGradMat(BYTE t);
	void Roberts(Mat image, double *kG);
	void RobertsMat(int T);
	void RobertsColor(Mat image, double *kG);
	void SobelMat(int T);
	void Sobel(Mat image, double *kG);
	void SobelColor(Mat image, double *kG);
	void CannyMat(double sig, double rl, double rh);
	void Canny(Mat image1,Mat image2,double sig,double rl,double rh);
	void GaussianSmooth(Mat image1, Mat image3,double sigma);
	void MakeGauss(double sigma, double **pdKernel, int *windowSize);
	void DirGrad(Mat image3, int *kGx, int *kGy);
	void GradMagnitude(int *kGx, int *kGy, int lWidth, int lHeight, int *GradM);
	void NonmaxSuppress(int *GradM, int *kGx, int *kGy, Mat image2);
	void Hysteresis(int *GradM, double rl, double rh,Mat image2);
	void EstimateThreshold(int *GradM, int *pnThdHigh, int *pnThdLow, Mat image2, double rh, double rl);
	void TraceEdge(int y, int x, int nLowThd, Mat image2, int *pnMag, int nWidth);
	void Laplacian(Mat image, double *kG);
	void LaplacianMat(int T);
	void LaplacianColor(Mat image, double *kG);
	
	void PriwittMat(int T);
	void Priwitt(Mat image, double *kG);
	void PriwittColor(Mat image, double *kG);
	void FUDIAO(int value);
	Mat regionGrow(Mat image, CPoint p, int T);
	Mat regionGrowcolor(Mat image, CPoint p, int T);
	...
	afx_msg void OnHsihuffman();
	afx_msg void OnShannonfano();
	afx_msg void OnAirth();
	afx_msg void OnRoberts();
	afx_msg void OnSobel();
	afx_msg void OnCanny();
	afx_msg void OnPrewitt();
	afx_msg void OnLaplacianseg();
	afx_msg void OnReimage();
	afx_msg void Onregiongrow();
	afx_msg void OnLButtonUp(UINT nFlags, CPoint point);
	afx_msg void OnReload();
	afx_msg void OnThreshold();
	afx_msg void OnFudiao();
	afx_msg void OnColorregiongrow();
	afx_msg void OnPencilroberts();
	afx_msg void OnPencilsobel();
	afx_msg void OnPencilpriwitt();
	afx_msg void OnFudiaolaplacian();
};
