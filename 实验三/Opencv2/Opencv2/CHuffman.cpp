...
CHuffman::CHuffman(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DIALOG_Huffman, pParent)
{
	m_dEntropy = 0.0;
	m_dCodLength = 0.0;
	m_dRatio = 0.0;
...
void CHuffman::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_AverageEDIT, m_dCodLength);
	DDX_Text(pDX, IDC_EncodevEDIT, m_dRatio);
...
BOOL CHuffman::OnInitDialog()
{
	...
	*******************************************************/
	...
	m_Cratio = 1 - m_dCodLength / 8;

	// 计算编码效率
	m_dRatio = m_dEntropy / m_dCodLength; //信息熵/码字长
	...
	*************************************************/
	...
	m_lstTable.InsertColumn(1, L"概率值", LVCFMT_LEFT, 78, 0);
	m_lstTable.InsertColumn(2, L"哈夫曼编码", LVCFMT_LEFT, 110, 1);
